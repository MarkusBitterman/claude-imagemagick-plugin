...
## v0.7 — sanity-check hook (done)

- [x] Hooks idea: PostToolUse hook that runs `magick identify` on any image file Claude writes, as an automatic sanity check
      — shipped as hooks/hooks.json (PostToolUse matcher "Bash") + hooks/identify-image.sh.
      Finds the output via the IM7 "output is the last argument" rule, gates on an image
      extension, and lets `magick identify` itself be the validity oracle (undecodable → warn,
      not a false alarm). Emits hookSpecificOutput.additionalContext with format/dims/depth/
      colorspace/size on success; systemMessage + context warning on 0-byte or undecodable;
      silent on non-image tokens, mogrify (no single output), and non-magick commands.
      jq-first, python3 fallback, no-op without both. Opt-out: IMAGEMAGICK_HOOK_DISABLE=1.
      Learnings (now in CLAUDE.md): `--plugin-dir` loads plugin hooks; acceptEdits does NOT
      auto-approve Bash (add `--allowedTools "Bash"`), and bypassPermissions is blocked by the
      auto-mode classifier for headless loops; PostToolUse hooks don't stream hook_started
      events (SessionStart does) — verify firing via a side-effect marker. Six hook cases
      added to the smoke test (161 base / 164 with nix).
- [ ] Confirm install + usage from a completely fresh conversation (user test drive)
- [~] Submit to the official directory: https://clau.de/plugin-directory-submission — BLOCKED (not a code issue).
      The submission page (claude.ai/admin-settings/directory/submissions/plugins/new) lives under
      organization settings, which require a Claude Team/Enterprise plan; an individual plan gets
      "You don't have access to organization settings." License ✓, tagged release ✓
      (`imagemagick--v0.7.0`), GitHub release published. The plugin installs and works via the README
      steps regardless — directory listing is discovery only, not distribution. Revisit if the
      account moves to Team/Enterprise.
- [x] Screenshot/GIF demo in README — `demo/` images generated by ImageMagick itself via `scripts/make-demo.sh`
      — extended: added tiny-planet.jpg (seamless panorama → Polar wrap), vignettes.jpg (4
      presets), distortions.jpg (Arc/swirl/implode/wave/Polar/Barrel on a grid). "Show, don't
      tell" now leads with built-in operators. Seamless-panorama recipe: purely vertical sky +
      ridgeline endpoints matched at x=0/x=W, or the Polar wrap scars at 12 o'clock; a hard sun
      disc near the horizon balloons into a blob in the ring — use a soft vertical warm gradient
      instead. README stats refreshed (155/158 smoke checks; Status v0.2→v0.6).
